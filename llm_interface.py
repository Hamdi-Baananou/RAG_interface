...
WEBSITE_CONFIGS = [
    {
        ...
        "pre_extraction_js": (
            ...
            "    if (searchResults) {"
            "        console.log('Found search results page, looking for exact part match...');"
            "        // Look for exact part number match in the results"
            "        const partLinks = Array.from(document.querySelectorAll('.search-result-item a'));"
            "        const exactMatch = partLinks.find(link => {"
            "            const partText = link.textContent.trim();"
            "            return partText.includes('" + "{part_number}" + "');"
            "        });"
            "        if (exactMatch) {"
            "            console.log('Found exact part match, clicking...');"
            "            exactMatch.click();"
            "            // Wait for part details page to load"
            "            await new Promise(r => setTimeout(r, 3000));"
            "        } else {"
            "            console.log('No exact part match found in search results');"
            "        }"
            "    } else {"
            "        console.log('Not on search results page, assuming we are on part details page');"
            "    }"
            "})();"
        ),
        "table_selector": ".technical-data-table, .product-details-table, .specifications-table, .product-specifications"
    },
    {
        "name": "Mouser",
